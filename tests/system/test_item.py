...
class ItemTest(BaseTest):
    ...
    def test_create_item(self):
        with self.app() as client, self.app_context():
            StoreModel('test').save_to_db()
            resp = client.post('/item/test', json={'price': 20.99, 'store_id': 1})
    ...
    def test_create_duplicate_item(self):
        with self.app() as client, self.app_context():
            StoreModel('test').save_to_db()
            client.post('/item/test', json={'price': 20.99, 'store_id': 1})
            resp = client.post('/item/test', json={'price': 20.99, 'store_id': 1})
    ...
    def test_update_existing_item(self):
        with self.app() as client, self.app_context():
            ...
            resp = client.put('/item/test', json={'price': 30.99, 'store_id': 1})

            self.assertEqual(resp.status_code, 200)
            self.assertEqual(ItemModel.find_by_name('test').price, 30.99)
            self.assertDictEqual({'name': 'test', 'price': 30.99},
                                 json.loads(resp.data))
